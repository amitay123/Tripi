...
class SecurityScreen extends StatelessWidget {
          ...
          children: [
            ...
            _buildSection(
              ...
              [
                ...
                _buildToggleItem(context, 'Two-Factor Auth',
                    'Secure your account with SMS or App', true),
              ],
            ),
      ...
      child: Row(
        children: [
          ...
          Icon(value ? Icons.check_circle : Icons.circle_outlined,
              color: value ? TripiColors.primary : TripiColors.outlineVariant),
        ],
      ),
    );
  }
  ...
  Widget _buildActiveSessions(BuildContext context) {
    return _buildSection(context, 'Active Sessions', [
      ...
      _buildSessionItem(
          'MacBook Pro M3', 'Paris, France • Last active 2h ago', false),
    ]);
  }
  ...
  Widget _buildSessionItem(String device, String details, bool isActive) {
    return Row(
      children: [
        Icon(isActive ? Icons.smartphone : Icons.laptop,
        ...
        if (isActive)
          ...
          const Icon(Icons.logout,
              size: 20, color: TripiColors.onSurfaceVariant),
      ],
    );
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(desc,
              style: const TextStyle(
                  color: TripiColors.onSurfaceVariant, fontSize: 12)),
          const SizedBox(height: 12),
          Text('$count Members',
              style: const TextStyle(
                  color: TripiColors.primary,
  ...
  Widget _buildActivityLog(BuildContext context) {
    return _buildSection(context, 'Security Activity Log', [
      _buildLogItem(
          'Successful Login - Admin Portal', 'alex.nav@travel.com', '12:45 PM'),
      _buildLogItem(
          '2FA Settings Changed', 'sarah.editor@travel.com', '10:30 AM'),
      _buildLogItem('Failed Password Attempt', 'Unknown User', '09:15 AM',
          isError: true),
    ]);
  }
  ...
  Widget _buildLogItem(String action, String user, String time,
    ...
    return Padding(
      ...
      child: Row(
        children: [
          Icon(isError ? Icons.error_outline : Icons.history,
              color: isError ? Colors.red : TripiColors.primary, size: 20),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
          ...
          Text(time,
              style: const TextStyle(
                  color: TripiColors.onSurfaceVariant, fontSize: 10)),
        ],
      ),
    );
  }
