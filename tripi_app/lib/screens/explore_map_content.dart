...
class _ExploreContentState extends State<ExploreContent> {
  ...
  List<String> _deduplicatedUrls(PlaceResult p) {
    ...
    return p.photoReferences
        .map((ref) => _placesService.getPhotoUrl(ref))
        .whereType<String>()
  ...
  Widget _buildPlaceDetailsSheet(PlaceResult place) {
    final urls = _deduplicatedUrls(place);
  ...
  Widget _buildTitle(PlaceResult p) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (p.formattedAddress != null || p.address != null) ...[
        const SizedBox(height: 10),
        Row(children: [
          const Icon(Icons.location_on_rounded, size: 15, color: TripiColors.primary),
          const SizedBox(width: 6),
          Expanded(child: Text(p.formattedAddress ?? p.address!, style: GoogleFonts.inter(
  ...
  Widget _buildAbout(PlaceResult p) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      ExpandableTextWidget(
        text: p.description!,
        storageKey: PageStorageKey('desc_explore_${p.placeId}'),
      ),
    ]);
  }
