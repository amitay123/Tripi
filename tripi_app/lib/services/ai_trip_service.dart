...
class AiTripService {
  ...
  Future<List<AiSuggestion>> generateDailyItinerary({
    ...
    final filtered = rawPlaces.where((p) {
      if (_visitedRegistry.isVisited(p['placeId'])) return false;
      if (_visitedRegistry.isVisitedByTitle(p['name'])) return false;
      
      final types = (p['types'] as List<String>);
      if (types.any((t) => _prefs.shouldAvoidType(t))) return false;
      ...
      if (options.familyFriendly && !(p['isFamilyFriendly'] as bool)) return false;
    ...
    double? anchorLat;
    double? anchorLng;
    if (trip.days[dayIndex].activities.isNotEmpty) {
      final last = trip.days[dayIndex].activities.last;
      anchorLat = last.lat;
      anchorLng = last.lng;
    }
  ...
  List<Map<String, dynamic>> _getMockPlaces(String city) {
    ...
    final database = {
      'London': [
        {'name': 'The British Museum', 'category': 'Museum', 'lat': 51.5194, 'lng': -0.1270, 'duration': 180, 'popularity': 4.9, 'types': ['museum'], 'isFamilyFriendly': true, 'imageUrl': 'https://images.unsplash.com/photo-1518709268805-4e9042af9f23?q=80&w=1000'},
        {'name': 'Borough Market', 'category': 'Market', 'lat': 51.5055, 'lng': -0.0910, 'duration': 90, 'popularity': 4.7, 'types': ['food', 'point_of_interest'], 'isFamilyFriendly': true, 'imageUrl': 'https://images.unsplash.com/photo-1533900298318-6b8da08a523e?q=80&w=1000'},
        {'name': 'Sky Garden', 'category': 'Viewpoint', 'lat': 51.5111, 'lng': -0.0837, 'duration': 60, 'popularity': 4.6, 'types': ['park', 'point_of_interest'], 'isFamilyFriendly': true, 'imageUrl': 'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?q=80&w=1000'},
        {'name': 'Dishoom Covent Garden', 'category': 'Restaurant', 'lat': 51.5124, 'lng': -0.1268, 'duration': 75, 'popularity': 4.8, 'types': ['restaurant', 'food'], 'isFamilyFriendly': true, 'imageUrl': 'https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?q=80&w=1000'},
        {'name': 'Tate Modern', 'category': 'Art Gallery', 'lat': 51.5076, 'lng': -0.0994, 'duration': 120, 'popularity': 4.5, 'types': ['art_gallery'], 'isFamilyFriendly': false, 'imageUrl': 'https://images.unsplash.com/photo-1549490349-8643362247b5?q=80&w=1000'},
        {'name': 'St. Paul\'s Cathedral', 'category': 'Historical Site', 'lat': 51.5138, 'lng': -0.0984, 'duration': 90, 'popularity': 4.7, 'types': ['church', 'place_of_worship'], 'isFamilyFriendly': true},
        {'name': 'Monmouth Coffee', 'category': 'Cafe', 'lat': 51.5144, 'lng': -0.1264, 'duration': 30, 'popularity': 4.6, 'types': ['cafe', 'food']},
      ],
      'Paris': [
        {'name': 'Louvre Museum', 'category': 'Museum', 'lat': 48.8606, 'lng': 2.3376, 'duration': 240, 'popularity': 4.9, 'types': ['museum'], 'isFamilyFriendly': true, 'imageUrl': 'https://images.unsplash.com/photo-1543349689-9a4d426bee8e?q=80&w=1000'},
        {'name': 'Tuileries Garden', 'category': 'Park', 'lat': 48.8635, 'lng': 2.3275, 'duration': 60, 'popularity': 4.6, 'types': ['park'], 'isFamilyFriendly': true},
        {'name': 'Le Comptoir de La Gastronomie', 'category': 'Restaurant', 'lat': 48.8643, 'lng': 2.3441, 'duration': 90, 'popularity': 4.5, 'types': ['restaurant', 'food'], 'isFamilyFriendly': false},
        {'name': 'Sainte-Chapelle', 'category': 'Historical Site', 'lat': 48.8555, 'lng': 2.3450, 'duration': 45, 'popularity': 4.8, 'types': ['church'], 'isFamilyFriendly': true},
        {'name': 'Café de Flore', 'category': 'Cafe', 'lat': 48.8542, 'lng': 2.3328, 'duration': 60, 'popularity': 4.2, 'types': ['cafe', 'food'], 'isFamilyFriendly': true},
      ]
    };

    return database[city] ?? database['London']!; // Default to London mock if city not found
  }
}
